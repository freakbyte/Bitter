...
namespace Bitter
{
    public class BinaryReader
    {
        ...
        /// <summary>
        /// Initializes a ReadWriteInitialize type object and then reads it from the stream.
        /// </summary>
        public T Type<T>(object parameters = null) where T : ReadWriteInitialize, new()
        {
            T ret = new T();
            ret.Init(parameters);
            ret.Read(stream);
        ...
        /// <summary>
        /// Initializes a List of ReadWriteInitialize type object and then reads it from the stream.
        /// </summary>
        public List<T> TypeList<T>(int count, object parameters = null) where T : ReadWriteInitialize, new()
        {
            List<T> ret = new List<T>(count);
            for (int i = 0; i < count; i++)
            {
        ...
        /// Initializes a List of ReadWriteInitialize type object and then reads it from the stream.
        /// </summary>
        public List<T> TypeList<T>(uint count, object parameters = null) where T : ReadWriteInitialize, new()
        {
            return TypeList<T>((int)count, parameters);
        }
